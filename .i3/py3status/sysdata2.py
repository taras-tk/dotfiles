...
class GetData:
    ...
    def memory(self):
        """
        Grab the memory capacity and used size from /proc/meminfo.
        Memory size 'total_mem', Used_mem, and percentage of used memory.
        """
        mem_data = self.execCMD('cat', '/proc/meminfo').split()
        mem_total = mem_data.index('MemTotal:')

        total_mem = int(mem_data[mem_total + 1]) / 1024. / 1024.

        #mem_avail = mem_data.index('MemAvailable:')
        #avail_mem = int(mem_data[mem_avail + 1]) / 1024. / 1024.

        mem_free = mem_data.index('MemFree:')
        avail_mem = int(mem_data[mem_free + 1])

        mem_buffers = mem_data.index('Buffers:')
        avail_mem += int(mem_data[mem_buffers + 1])

        mem_cached = mem_data.index('Cached:')
        avail_mem += int(mem_data[mem_cached + 1])
        avail_mem /= 1024. * 1024.
